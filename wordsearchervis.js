...
const wordSearcherAlgorithm = (word, array, arrayN, arrayM) =>{     
    ...
    // list which stores all match arrays for visualizer function 
    let tracker = []; 
    ...
    for (let i = 0; i < arrayN; i++){
        for (let j = 0; j < arrayM; j++){ 
            ...
            if (horizSearch(i, j, idx, array, arrayM, word, matched) ){
                tracker.push({i, j , matched}); 
                return tracker; 
            }
            idx = 0; // reset idx 
            // append to tracker so that we always have a 'cursor' in the animation 
            tracker.push({i, j, matched}); 
            ...
            if(matched.length > 0 ){tracker.push({i, j, matched}) } 
    ...
    return tracker; // did not find word  
    
}
const wordSearchAlgorithmVisualizer = (tracker, tableArray, speed) =>{
    let idx = 0;
    let matchedIdx = 0; 
    let lastMatched = [];  
    let animation = setInterval( () =>{
        if (idx >= tracker.length){
            clearInterval(animation);
            return; 
        }
        const curRow = tracker[idx].i;  
        const curCol = tracker[idx].j;
        // reset styling to none from previous cursor
        if (idx> 0){
            tableArray[tracker[idx -1].i ][ tracker[idx -1].j ].style.backgroundColor = null; 
        }
        // color cur row and col with yellow cursor 
        tableArray[curRow][curCol].style.backgroundColor = 'yellow';
        // style matched from matchin algorithm 
        const matched = tracker[idx].matched; 
        if (matchedIdx < matched.length){

            tableArray[ matched[matchedIdx][0] ][ matched[matchedIdx][1] ].style.border = '1px solid';
            lastMatched.push(matched[matchedIdx]);
            matchedIdx++;
            
        }else{
            if (lastMatched.length > 0){ 
                // undo border styling 
                 lastMatched.forEach( ([i, j]) => tableArray[i][j].style.border = null); 
            }
            lastMatched = []; 
            matchedIdx = 0; 
            idx++; 
...
window.onload = () =>{
    ...
    wordSearcherButton.onclick = () =>{
        ...
        if (wordSearchValue){
            wordSearchAlgorithmVisualizer(wordSearchValue, wordSearcherTableArray, 150);  
            // wordSearchAlgorithmVisualizer(wordSearchValue, wordSearcherTableArray); 
            // clear previous error output 
            document.getElementById("output").textContent = '';
            return; 
        }
        document.getElementById("output").textContent = `Could not find word '${searchWord}'`; 
    }
